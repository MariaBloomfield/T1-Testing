...
class ExtractSetupTransformer(NodeTransformer):
    ...
    def visit_ClassDef(self, node):
        ...
        if len(self.variables_duplicadas) > 0:
            for variable, value in self.variables_duplicadas:
                assign_stmt = Assign(
                    targets=[Attribute(value=Name(id='self', ctx=Load()), attr=variable, ctx=Store())],
                    value=Num(n=value),
                )
            ...
            if not self.should_remove_node(statement):
                ...
                if isinstance(statement, ast.Assign):
                    ...
                    for target in statement.targets:
                        if isinstance(target, ast.Name) and target.id in self.variables_duplicadas:
                ...
                if isinstance(statement, ast.Expr) and isinstance(statement.value, ast.Call):
                    print("Iprimiendo", statement.value)
    ...
    def replace_variables_in_call(self, call_node):
        if isinstance(call_node, ast.Call):
            # Reemplazar nombres de variables en los argumentos de la llamada
            new_args = []
            for arg in call_node.args:
                if isinstance(arg, ast.Name):
                    # Si el argumento es un nombre de variable, verifica si está en la lista de duplicados
                    for nombre, value in self.variables_duplicadas:
                        if arg.id == nombre:
                        ...
                        # Si no se encuentra en la lista de duplicados, mantenlo sin cambios
                        new_args.append(arg)
                else:
                    # Si no es un nombre de variable, mantenlo sin cambios
                    new_args.append(arg)
        ...
        if isinstance(node, ast.Assign):
            ...
            for target in node.targets:
                ...
                if isinstance(target, ast.Name):
                    for target.id, valor in self.variables_duplicadas:
